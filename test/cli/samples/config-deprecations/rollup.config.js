	...
	onwarn (warning) {
		...
		if (warnings.length === 1) {
			...
			assert.deepEqual(
				...
				{
					code: 'UNKNOWN_OPTION',
					message: 'Unknown option found: abc. Allowed keys: acorn, acornInjectPlugins, cache, context, experimentalCodeSplitting, experimentalDynamicImport, input, legacy, moduleContext, onwarn, perf, plugins, preferConst, preserveSymlinks, treeshake, watch, entry, external, amd, banner, dir, exports, extend, file, footer, format, freeze, globals, indent, interop, intro, legacy, name, noConflict, outro, paths, sourcemap, sourcemapFile, strict, pureExternalModules'
				}
			);
		} else {
